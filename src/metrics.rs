...
impl MetricsRegistry {
    ...
    pub fn new() -> Result<Self> {
        ...
        // Commitment metrics
        let commitments_total = IntGaugeVec::new(
            Opts::new("gateway_commitments_total", "Total number of commitments"),
            &["type"]
        )?;

        let commitments_by_type = IntGaugeVec::new(
            Opts::new("gateway_commitments_by_type", "Commitments by type"),
            &["commitment_type"]
        )?;

        // Delegation metrics
        let delegations_total = IntGaugeVec::new(
            Opts::new("gateway_delegations_total", "Total delegations"),
            &["status"]
        )?;

        let delegations_active = IntGaugeVec::new(
            Opts::new("gateway_delegations_active", "Active delegations count"),
            &["label"]
        )?;

        let unique_proposers = IntGaugeVec::new(
            Opts::new("gateway_unique_proposers", "Unique proposers count"),
            &["label"]
        )?;

        let unique_delegates = IntGaugeVec::new(
            Opts::new("gateway_unique_delegates", "Unique delegates count"),
            &["label"]
        )?;

        let slots_covered = IntGaugeVec::new(
            Opts::new("gateway_slots_covered", "Number of slots with delegations"),
            &["label"]
        )?;
        ...
        let highest_congestion = GaugeVec::new(
            Opts::new("gateway_highest_congestion_ratio", "Highest congestion ratio"),
            &["window"]
        )?;

        let average_fee_multiplier = GaugeVec::new(
            Opts::new("gateway_average_fee_multiplier", "Average fee multiplier"),
            &["window"]
        )?;

        // Pricing metrics
        let current_slot = IntGaugeVec::new(
            Opts::new("gateway_current_slot", "Current beacon chain slot"),
            &["label"]
        )?;

        let base_gas_price = IntGaugeVec::new(
            Opts::new("gateway_base_gas_price_wei", "Current base gas price in wei"),
            &["label"]
        )?;
