...
impl PayloadParser {
	...
	pub fn extract_slot(commitment_type: u64, payload: &[u8]) -> Result<u64> {
		match commitment_type {
			...
			_ => Err(anyhow::anyhow!("Unknown commitment type {} for slot extraction", commitment_type)),
		}
	}
	...
	pub fn parse_inclusion_payload(payload: &[u8]) -> Result<InclusionPayload> {
		...
		Self::parse_inclusion_payload_rlp(payload).context("Failed to parse inclusion payload as both JSON and RLP")
	}
	...
	fn parse_inclusion_payload_rlp(payload: &[u8]) -> Result<InclusionPayload> {
		...
		let slot: u64 = rlp.val_at(0).context("Failed to decode slot from RLP")?;

		let signed_tx: Vec<u8> = rlp.val_at(1).context("Failed to decode signed_tx from RLP")?;

		Ok(InclusionPayload { slot, signed_tx })
	}
